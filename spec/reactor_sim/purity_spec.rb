...
# The keystone spec. Everything in docs/architecture.md §6 and §8 — exact crash
# recovery, replay, spectating — rests on the simulation being a self-contained,
# side-effect-free library. This spec is what stops that from being merely an
# intention someone wrote down once.
RSpec.describe "ReactorSim purity" do
    ...
    it "loads in a bare Ruby process with no Rails present" do
      script = <<~RUBY
        ...
        match = ReactorSim::Match.create(
          id: "purity", seed: 1, operations: [{ id: :vats, type: :chemical_vats }]
        )
        10.times { match.step! }
  ...
  describe "source" do
    # A static sweep, because the load test above only catches what a particular code
    # path happens to touch. These are the specific things that would silently destroy
    # reproducibility.
    FORBIDDEN = {
    ...
    it "requires nothing outside the simulation but json" do
      requires = sim_sources.flat_map do |path|
        File.readlines(path).grep(/^\s*require\s+["']/).map { |l| l[/["']([^"']+)["']/, 1] }
      end

      expect(requires.uniq).to contain_exactly("json")
    end
  end
  ...
  # Strip Bundler out of the child's environment so the subprocess really is a bare
  # Ruby process rather than one already inside the app's gem context.
  def clean_env
